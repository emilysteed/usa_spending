...
def get_usaid_term_data():
    ...
    usaid_term_data["state_abbrev"] = (
        usaid_term_data["Prime State"].str.title().map(us_state_to_abbrev)
    )

    # Create count variable to generate counts of terminations by state
    usaid_term_data["state_term_count"] = usaid_term_data["state_abbrev"].map(
        usaid_term_data["state_abbrev"].value_counts()
    )

    # Reformat Obligation variable
    usaid_term_data["Obligation"] = (
        usaid_term_data["Obligation"].replace("\$|,", "", regex=True).astype(float)
    )

    # Create sum variable to sum total amount obligated by state
    usaid_term_data["state_oblg_funds"] = (
        usaid_term_data["Obligation"]
        .groupby(usaid_term_data["state_abbrev"])
        .transform("sum")
