                ...
                dbc.Col(
                    dbc.Card(
                        dbc.CardBody(
                            [
                                html.H2(
                                    "USAID Terminations", className="title-card-large"
                                ),
                            ]
                        ),
...
@callback(
    [Output("map-usa", "figure"), Output("chart-state", "figure")],
    [Input("state-dropdown", "value"), Input("prime-dropdown", "value")],
)
def update_values(select_state, select_prime):
    ...
    if select_state and "All" not in select_state:
        df = df[df["state_abbrev"].isin(select_state)]

    # Filter by selected primes (handle multi-selection)
    if select_prime and "All" not in select_prime:
        df = df[df["Prime"].isin(select_prime)]

    # Call the functions to generate the figures
    map_figure = usaid_term_map_usa(df)
    chart_figure = usaid_term_graph_state(df)
